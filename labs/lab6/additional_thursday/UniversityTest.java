package labs.lab6.additional_thursday;
...
class University {
    ...
    public University mergeFourSmallestDepartments() {
        ...
        // Sort departments by total enrolled students and get the 4 smallest
        ...
        // Merge the four smallest departments using reduce
        Department merged = fourSmallest.stream()
                .reduce((dept1, dept2) -> new Department(
                        dept1.getName() + " & " + dept2.getName(),
                        Stream.concat(
                                dept1.getCourses().stream(),
                                dept2.getCourses().stream()
                        ).collect(Collectors.toList())
                )).orElse(new Department("Empty", new ArrayList<>()));

        // Create new department list: remaining departments + merged department
        List<Department> newDepartments = departments.stream()
                .filter(dept -> !fourSmallest.contains(dept))
                .collect(Collectors.toList());
        newDepartments.add(merged);
    ...
    //TODO: Additional method 1
    public List<Course> getCoursesWithNameContaining(String substring) {
        return departments.stream()
                .flatMap(department -> department.getCourses().stream())
                .filter(course -> course.getName().contains(substring))
                .collect(Collectors.toList());
    }

    //TODO: Additional method 2
...
public class UniversityTest {
    public static void main(String[] args) {
        ...
        } else if (method.equals("getCoursesWithNameContaining")) { //TODO:METHOD 1 TEST
            System.out.println("Input a substring:");
            String substr = sc.next();
            System.out.println(university.getCoursesWithNameContaining(substr));
