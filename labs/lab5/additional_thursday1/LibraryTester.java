package labs.lab5.additional_thursday1;
 ...
 * getTopBookPerAuthor(): Map<String, Book>
 * which returns a map where the key is the author's name and the value is the book with the highest number of borrowings from that author.
 *
 * Implement the method
 * getBooksWaitingListSize(): Map<String, Integer>
 * which returns a map where the key is the book’s ISBN number,
 * and the value is the number of members waiting for that book (the size of the waiting list).
 * The resulting map should be sorted by the waiting list size (descending), and if equal, sorted by ISBN (ascending).
 */
...
class LibrarySystem {
    ...
    public Map<String, Book> getTopBookPerAuthor() {
        ...
        for (Book book: books.values()) {
            ...
            // If author not in map, or current book has more borrows, update
            if (!result.containsKey(author) || book.getTotalBorrows() > result.get(author).getTotalBorrows()) {
                result.put(author, book);
            }
        }
    ...
    public Map<String, Integer> getBooksWaitingListSize() {
        ...
        // sort and return as LinkedHashMap
        return result.entrySet().stream()
                .sorted(Map.Entry.<String, Integer>comparingByValue().reversed()
                        .thenComparing(Map.Entry.comparingByKey()))
                .collect(Collectors.toMap(
                        Map.Entry::getKey,              // ISBN
                        Map.Entry::getValue,            // size of the queue
                        (v1, v2) -> v1,  // merge function (not needed)
                        LinkedHashMap::new // // preserves insertion order from sorted stream
                ));
    }
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                switch (parts[0]) {
                    ...
                    case "printBooks": {
                        lib.printBooks();
                        System.out.println("Additional method 2: " + lib.getBooksWaitingListSize()); // TODO:Method 2
                        break;
                    }
                    ...
                    case "printTopAuthors": {
                        lib.printTopAuthors();
                        System.out.println("Additional method 1: " + lib.getTopBookPerAuthor()); //TODO:Method 1
                        break;
                    }
